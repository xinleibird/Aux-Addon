...
function SlashCmdList.AUX(command)
	...
	elseif arguments[1] == 'show' and arguments[2] == 'hidden' then
		...
		aux.print('- post bid [' .. status(aux.account_data.post_bid) .. '] 在上拍选项卡中添加出价清单')
		...
		aux.print('- tooltip disenchant distribution [' .. status(tooltip_settings.disenchant_distribution) .. '] 鼠标提示 分解分布')
